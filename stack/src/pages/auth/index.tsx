...
const index = () => {
  ...
  const handleOtpSubmit = async (e: any) => {
    ...
    if (!loginChallenge) {
      toast.error("Please restart login.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={loginChallenge ? handleOtpSubmit : handlesubmit}>
          <Card>
            <CardHeader className="space-y-1 text-center">
              <CardTitle className="text-xl lg:text-2xl">
                {loginChallenge ? "Verify Chrome login" : t("auth.loginTitle")}
              </CardTitle>
              <CardDescription>
                {loginChallenge
                  ? `Enter the OTP sent to ${loginChallenge.maskedEmail}. It expires in ${loginChallenge.expiresInMinutes} minutes.`
              ...
              {loginChallenge ? (
                <>
                  <div className="rounded border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
                    Google Chrome requires email OTP verification before login.
                  </div>
